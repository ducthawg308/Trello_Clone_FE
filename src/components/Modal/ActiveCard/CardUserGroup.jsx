...
function CardUserGroup({ cardMemberIds = [], onUpdateCardMember}) {
  ...
  // Lấy activeBoard trong redux để lấy được toàn bộ member thuộc về board thông qua FE_allUsers
  const board = useSelector(selectCurrentActiveBoard)

  const FE_CardMembers = cardMemberIds.map(id => board.FE_allUsers.find(u => u._id === id))
  
  const handleUpdateCardMembers = (user) => {
    // Tạo một biến incomingMemberInfo để gửi cho BE, với thông tin chính là userId và action là xóa khỏi card hoặc thêm vòa card
