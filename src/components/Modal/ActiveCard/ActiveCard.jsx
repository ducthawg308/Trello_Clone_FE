...
const SidebarItem = styled(Box)(({ theme }) => ({
  ...
  '&.active:hover': {
    // optional: nếu muốn hover vào item active khác đi
    ...theme.applyStyles('dark', {
      backgroundColor: '#90caf9'
    })
  }
...
const ActiveCard = () => {
  ...
  // Function dùng chung cho các hành động liên quan đến update card
  const callApiUpdateCard = async (updateData) => {
    const updatedCard = await updateCardDetailsAPI(activeCard._id, updateData)
    
    // B1: cập nhật lại cái card đang active trong redux
    dispatch(updateCurrentActiveCard(updatedCard))
    // B2: cập nhật lại cái bản ghi card trong activeBoard
  ...
  const onUpdateCardMember = async (incomingMemberInfo) => {
        ...
        <Grid container spacing={2} sx={{ mb: 3 }}>
          ...
          <Grid size={{ xs: 12, sm: 3 }}>
            ...
            <Stack direction="column" spacing={1}>
              ...
              {/* Nếu current chưa là thành viên của Card thì mới hiện nút Join ra, bởi vì khi nhấn vào nút Join thì hành động luôn là ADD */}
